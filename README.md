...
без каких-либо дополнительных модулей (использовался модуль http.server). При решении использовалась версия Python 3.6.9.


Пример GET-запроса: ```http://localhost:8000/?sku=2rFeIfZiyD&rank=0.8```


Параметр rank опциональный, если он отсутствует, то по умолчанию равен 0.
...
Далее добавляю в список значения, которые больше или равны переданному rank и возвращаю его в json-формате.


Также я постарался учесть ситуации, когда sku не передан, передан несуществующуй sku, некорректный rank и т.д.
